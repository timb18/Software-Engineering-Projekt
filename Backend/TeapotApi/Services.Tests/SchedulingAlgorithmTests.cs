...
public class SchedulingAlgorithmTests
{
    ...
    private DependencyAnalysisResult Analyze(
        ...
        => _analyzer.Analyze(tasks, deps ?? [], T0);
    ...
    {
        var id = Guid.NewGuid();
        var task = MakeTask(id, estimateMinutes: 60);
    ...
    {
        var tasks = Enumerable.Range(0, 4).Select(_ => MakeTask(Guid.NewGuid(), 60)).ToList();
    ...
    public void Schedule_LargeTask_SplitsAcrossMultipleSlots()
    {
        ...
        var id = Guid.NewGuid();
        var task = MakeTask(id, estimateMinutes: 600);

        var blocks = Schedule([task], MakeSlots(days: 7, minutesPerDay: 480));
    ...
    {
        var tasks = Enumerable.Range(0, 3).Select(_ => MakeTask(Guid.NewGuid(), 60)).ToList();
    ...
    public void Schedule_MultipleTasks_BlocksDoNotOverlap()
    {
        ...
        var blocks = Schedule(tasks, MakeSlots(days: 7, minutesPerDay: 480))
    ...
    public void Schedule_DependentTasks_PredecessorScheduledFirst()
    {
        ...
        var tasks = new List<UserTask> { MakeTask(aId, 60), MakeTask(bId, 60) };
    ...
    public void Schedule_AlreadyScheduledPredecessor_DependentBecomesReady()
    {
        ...
        var aId = Guid.NewGuid();
        var bId = Guid.NewGuid();
        var taskA = MakeTask(aId, 60);
        var taskB = MakeTask(bId, 60);
        var deps = new List<TaskDependency> { new() { TaskId = bId, DependsOnTaskId = aId } };
    ...
    public void Schedule_TaskWithDeadline_AllBlocksBeforeDeadlineMidnight()
    {
        ...
        var task = MakeTask(id, 60, deadline: deadline);
    ...
    public void Schedule_EarlierDeadlineFirst_UrgentTaskScheduledBeforeRelaxedTask()
    {
        ...
        var urgent  = MakeTask(urgentId,  60, deadline: T0.AddDays(1));
        var relaxed = MakeTask(relaxedId, 60, deadline: T0.AddDays(5));
    ...
    public void Schedule_NoDeadlines_HighPriorityScheduledFirst()
    {
        ...
        var high = MakeTask(highId, 60, priority: ETaskPriority.High);
        var low  = MakeTask(lowId,  60, priority: ETaskPriority.Low);
    ...
    public void Schedule_PartialPastWork_OnlyRemainingMinutesScheduled()
    {
        ...
        var id = Guid.NewGuid();
        var task = MakeTask(id, estimateMinutes: 120);
    ...
    public void Schedule_DeadlineWithTimeOfDay_BlocksEndAtOrBeforeDeadline()
    {
        ...
        var task = MakeTask(id, estimateMinutes: 240, deadline: deadline);
    ...
    public void Schedule_IntensiveTask_LeavesRecoveryGapBeforeNextTask()
    {
        ...
        var intensive = MakeTask(intensiveId, estimateMinutes: 60);
        ...
        var normal = MakeTask(normalId, estimateMinutes: 60);
    ...
    public void Schedule_LongNormalTask_SplitsInto90MinBlocksWith15MinBreaks()
    {
        ...
        var id = Guid.NewGuid();
        var task = MakeTask(id, estimateMinutes: 240);
    ...
    public void Schedule_LongLightTask_NotSplit_NoBreaks()
    {
        ...
        var id = Guid.NewGuid();
        var task = MakeTask(id, estimateMinutes: 300);
    ...
    public void Schedule_TwoNormalTasksInSameSlot_HaveBreakBetweenThem()
    {
        ...
        var aId = Guid.NewGuid();
        var bId = Guid.NewGuid();
        var a = MakeTask(aId, estimateMinutes: 60);
        a.Priority = ETaskPriority.High;
        var b = MakeTask(bId, estimateMinutes: 60);
