...
public class MembershipService(IMembershipRepository membershipRepository) : IMembershipService
{
    ...
    /// </summary>
    public async Task LeaveOrganizationAsync(Guid userId, Guid organizationId,
    ...
    /// </summary>
    public async Task RemoveUserFromOrganizationAsync(Guid initiatorUserId, Guid userId, Guid organizationId,
        CancellationToken cancellationToken = default)
    {
        var initiatorMembership =
            await membershipRepository.FindOrganizerAsync(organizationId, initiatorUserId, cancellationToken)
    ...
    /// </summary>
    public async Task UpdateRoleAsync(Guid initiatorUserId, Guid userId, Guid organizationId, string role,
        CancellationToken cancellationToken = default)
    {
    ...
    private async Task RemoveMembershipAsync(Guid userId, Guid organizationId,
        CancellationToken cancellationToken = default)
    {
        if (userId == Guid.Empty)
            throw new ArgumentException("UserId is required.", nameof(userId));
        ...
        if (membership.Role == ERole.Organizer)
        {
            ...
            if (organizerCount <= 1)
                throw new InvalidOperationException(
                    "Cannot leave: you are the only organizer of this organization. Transfer ownership first or delete the organization.");
        }
    ...
    {
        return (role?.Trim().ToLowerInvariant() ?? string.Empty) switch
        {
