...
const EditTaskModal: FC<EditTaskModalProps> = ({ task, onClose }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await updateTask(workProfileId!, task.id!, newTask, token);
      await fetchTasks(workProfileId!, token).then((fresh) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = await getAccessTokenSilently();
      await deleteTask(workProfileId!, task.id!, token);
  ...
  return createPortal(
    ...
    >
      <div
        className="flex max-h-screen w-full max-w-lg flex-col rounded-3xl bg-slate-900 p-6 shadow-xl"
        ...
        <div className="p-1">
          <div className="mt-4 flex justify-end gap-2">
            <button
              ...
              className="mr-auto rounded-xl border border-red-300/60 bg-red-500/10 py-1 font-semibold text-red-100 hover:bg-red-500/20"
            >
              Delete
            </button>
            <button
