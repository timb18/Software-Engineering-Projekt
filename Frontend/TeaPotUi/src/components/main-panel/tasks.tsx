...
const Tasks: FC = () => {
  ...
  const personalOrg = useMemo<Org>(
    () =>
      user.orgs?.[0] ?? {
        id: `personal-${user.email}`,
        name: "Personal",
        users: [user],
        invites: [],
      },
  ...
  const submitTask = () => {
    ...
    const newTask: Task = {
      ...
      org: findOrgForEmail(selectedAssignee),
      recurrence: "none",
      dependencies,
    };

    const conflicts = (user.tasks ?? []).filter((t) => {
      if (!t.org.users.some((member) => member.email === selectedAssignee)) return false;
    ...
    const updatedTasks = (user.tasks ?? []).map((t) =>
      t === editingTask
        ? {
            ...
            org: findOrgForEmail(editForm.assigneeEmail),
